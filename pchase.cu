/// The following are a number renditions of the P-Chase algorithm from a number of publications:
...
__global__ void l1_bw(uint32_t * startClk, uint32_t * stopClk, double * dsink, uint32_t * posArray){ // thread index
...
double * ptr = posArray+i; asm volatile ("{\t\n"
