...
__global__ void global_latency(READ_TYPE *my_array, unsigned array_length,
                               unsigned iterations, int *duration,
                               READ_TYPE *index) {

  __shared__ int s_tvalue[READ_COUNT];
  ...
  // WARM UP
  READ_TYPE j = 0;
  for (unsigned k = 0; k < iterations * READ_COUNT; k++) {
    j = my_array[j];
  }

  // P-Chase
  j = 0;
  for (int k = -iterations * 512; k < iterations * READ_COUNT; k++) {
    if (k >= 0) {
...
void parametric_measure_global(int N, int iterations, int stride) {
  ...
  auto duration = DeviceMalloc<READ_TYPE>(READ_COUNT);
...
auto pchase_mei_host(int argc, char **argv) -> int {
  ...
  int stride = 128 / sizeof(READ_TYPE);

  int START_N = 16 * 256;
  int END_N = 24 * 256;

  for (int N = START_N; N <= END_N; N += stride) {
